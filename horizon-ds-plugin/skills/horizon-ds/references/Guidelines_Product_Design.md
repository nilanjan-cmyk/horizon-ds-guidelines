...
| **Breadcrumb** | Horizontal path trail, placed below the Top Nav and above the Page Header. Optionally prefixed by the Dashboard Button (sidebar toggle). |
...
```yaml
figma_page: "714:41570"
location: "Below the global nav bar, above the Page Header. Use on pages 2+ levels deep."
typography: "type/sm/normal/medium"  # 14/20/500

sub_components:                       # 4 published symbols
  Breadcrumb:                         # the full bar
    props: "Size=md|sm · Sidebar=Open|Collapsed · Show Item 1–10"
  Breadcrumb/Item:
    props: "Label · Show Icon · State=Default|Hover|Current"
    height: 20
    current: "the active page — plain text, never a link, no separator after it"
  Breadcrumb/Separator:
    props: "Type=Chevron|Slash"
    chevron: "14x18 — directional parent → child hierarchy (preferred in-app)"
    slash:   "5x21 — URL-style; use when the trail mirrors the URL"
    rule: "one separator type per trail — never mix"
  Breadcrumb/Dashboard Button:
    props: "Sidebar=Open|Collapsed|Back"
    size: "40x40"
    placement: "always leftmost, before Item 1"
    rule: "only in layouts with a collapsible sidebar; replaces the nav-bar sidebar toggle — never ship both"

sizes:
  md: "standard — desktop page headers"
  sm: "compact — dense layouts, secondary panels"

separator_color: "typography/text-secondary"
  # Correction: previous docs said typography/text-tertiary.

item_gap: 20  # px — item ↔ separator (Figma anatomy). Legacy docs said 8.
...
item_count: "max 5 fully visible; deeper trails collapse middle items into an … dropdown. Always show root + current."
...
do:
  - "Mark the current page with State=Current, never as a link."
  - "Pick one separator type per trail and keep it consistent."
  - "Keep the Sidebar prop in sync with real sidebar state."
dont:
  - "Don't make the Current item clickable."
  - "Don't mix Chevron and Slash in one trail."
  - "Don't show more than 5–6 items without ellipsis truncation."
```
...
### 13.43 Announcement Popup

```yaml
figma_page: "7191:1631"
size: { w: 568, h: 588 }
purpose: "One-time feature announcement shown on first load after a release."

bg: "surface/card-background"
media_bg: "surface/widget-background"
radius: "radius/rounded_6 = 24"       # inner blocks radius/rounded_2 = 8
shadow: "Box Shadow/shadow-lg"
padding: 24                            # spacing_gap/gap_6
gap: { section: 24, stack: 16, inline: 12, tight: 8 }

ribbon:
  text: "New Feature !!"
  placement: "45° corner ribbon, top-left"
  style: "type/xs/normal/regular on gold/amber fill, typography/text-white"
  note: "decorative fill — not a semantic token"

media:
  region: "emerald-tinted gradient wash (bv-emerald-50 → base-white), radius/rounded_2"
  carousel: "dots below media · active = bv-emerald-700 pill, inactive = Flat/zinc/zinc-200 dot (rounded-full)"

title:
  style: "type/lg/normal/medium"       # 18/28/500
  color: "typography/text-primary"
  accent: "feature name set italic in Flat/emerald/emerald-600, inline"
description:
  style: "type/xs/normal/regular"      # 12/16/400, max 2 lines, centered
  color: "typography/text-secondary"

buttons:
  layout: "right-aligned pair, gap_3 = 12, both md"
  secondary: "Ghost — typography/text-primary label + leading icon"
  primary: "BvButton Primary — bv-emerald-700 fill, base-white label, radius/rounded_2"
  label_style: "type/sm/normal/medium"
...
disabled: N/A
focus: "close + both CTAs tab-focusable; trap focus while open"
animation: "fade + scale-in 200ms ease-out; carousel slide 240ms"

do:
  - "Show at most once per release, per user."
  - "Keep the description to two lines — link out for detail."
dont:
  - "Don't stack it with a Modal or Dialog — one blocking surface at a time."
  - "Don't use it for errors, confirmations or consent — that's Dialog / Feedback."
```
